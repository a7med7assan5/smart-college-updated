    ...
    try {
        ...
        if (usercourses) {
            ...
            for (let y = 0; y < usercourses.length; y++) {
                let x = await teacherService.getCourseData(arr[y].Id);
                if (x) {
                    let data = arr[y];
                    let courseDate = { data, x }
                    arr2[y] = courseDate
                }
            }
            res.json(arr2);
...
exports.addSemesterLecture = async (req, res, next) => { //[]
    const { lectureNumber, routerAddress } = req.body
    ...
    try {
        ...
        if (!checkCourseId) {
            ...
            teacherService.addLecture(courseCode, semester_time, lectureNumber, routerAddress).then((lecture) => {
                if (lecture) {
...
exports.addSemesterAttendance = async (req, res, next) => { //[]
    ...
    try {
        ...
        let checkbeacon_id = await Course.findOne({
            ...
            'semesters.semester_time': semester_time,
            'semesters.lectures.lectureNumber': { $in: lectureNumber },
            'semesters.lectures.routerAddress': { $in: routerAddress }
        }, { semesters: { $elemMatch: { semester_time: semester_time } } });
        if (!checkCourseId) {
            return res.status(400).json({
        ...
        if (checkAttendance) {
            ...
            for (var i = 0; i < Students.length; i++) {
                let stu = Students[i];
                teacherService.addSemesterAttendance(stu._id, courseId, semester_time, lectureNumber, routerAddress);
            }
            res.status(200).json({
                icon: '&#xE876;',
                style: 'success',
...
exports.semesterAttendme = async (req, res, next) => { //[]
    ...
    let routerAddress = req.body.routerAddress;
    let semester_time = req.params.semester;
    try {
        ...
        if (!checkbeacon_id) {
            ...
            teacherService.attendme(studentId, courseId, semester_time, lectureNumber, routerAddress).then((data) => {
                if (data) {

...
exports.addUserCourse = async (req, res, next) => { //[]
    ...
    try {
        let checkforActionUser = await User.findOne({
            _id: senderId
        });
        let checkforUser = await User.findOne({
            _id: id
        });
        let checkforUserNewCourses = await User.aggregate([
            // find the relevant documents in the collection
            // uses index, if defined on documents.x
            { $match: { _id: id, courses: { $elemMatch: { "status": 'new' } } } },
            // flatten array documennts
            { $unwind: "$courses" },
            // match for elements, "documents" is no longer an array
            { $match: { _id: id, "courses.status": "new" } },
            // re-create documents array
        ]);
        if (checkforActionUser.role == "admin" && checkforUserNewCourses > 7) {
            return res.status(400).json({
                icon: '&#xE5CD;',
                style: 'error',
                msg: "This User Can't Register More Than 7 Courses In A Term"
            });
        }
        else if (checkforActionUser.role == "student" && checkforUserNewCourses > 6) {
            return res.status(400).json({
                icon: '&#xE5CD;',
                style: 'error',
                msg: "You Can't Register More Than 6 Courses In A Term"
            });
        }
        else if (checkforUser.role == 'student') {
...
exports.deleteUserCourse = async (req, res, next) => { //[]
    ...
    try {
        ...
        if (!checkforUser) {
            ...
            let deleteUserGrades = await Grade.deleteMany({ courseId: courseCode, studentId: id })
            let deleteUserAttendance = await Attendance.deleteMany({ courseId: courseCode, studentId: id })
...
exports.updateSemesterGrade = async (req, res, next) => { //[]
    ...
    let semester_time = req.params.semester;
    try {
        let checkStudentId = await User.findOne({
            _id: studentId
        });
...
exports.addRouter = async (req, res, next) => { //[]
    try {
        const { locationId, locationName, routerAddress } = req.body
        const checkLocationId = await Router.findOne({ locationId });
        const checkLocationName = await Router.findOne({ locationName });
        if (checkLocationId) {
            res.status(400).json({ msg: 'This Location ID Has Been Used Before' })
        } else if (checkLocationName) {
            res.status(400).json({ msg: 'This Location Name Has Been Used Before' })
        } else {
            const newLocation = new Router({ locationId, locationName, routerAddress });
            await newLocation.save();
            res.status(200).json({ msg: 'Router Added Successfuly' })
        }
    } catch (error) {
        next(error)
    }
...
exports.updateRouter = async (req, res, next) => { //[]
    ...
    try {
        ...
        if (!checkLocationId) {
            ...
            Router.findOneAndUpdate({ locationId },
                req.body, { useFindAndModify: false },
                (err) => {
                    if (err) {
                        res.status(404).json({ msg: "Can't Update this Router Information" });
                    }
                    res.status(201).json({ msg: "Router's Information Updated Successfuly" });
                });
        }
    } catch (err) {
        console.log(err.message);
...
exports.deleteRouter = async (req, res, next) => { //[]
    ...
    try {
        ...
        if (!checkLocationId) {
            ...
            let deleteRouter = await Router.deleteOne({ locationId });
            if (deleteRouter) {
                res.status(201).json({ msg: "Router's Information Deleted Successfuly" });
            }
        }
    } catch (err) {
        console.log(err.message);
...
// --------------------Get Courses---------------------
exports.getAllRouters = async (req, res, next) => { //[]
    adminService.getAllRouters().then((routers) => {
        if (routers) {
            res.json(routers);
        } else {
            res.status(404).json({ msg: 'No Routers Yet' });
        }
    }).catch(err => {
        console.log(err);
        res.status(500).json({ msg: 'Internal Server Error' });
